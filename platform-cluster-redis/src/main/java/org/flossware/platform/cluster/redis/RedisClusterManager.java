...
/**
 * Redis-based ClusterManager implementation. Provides clustering via Redis distributed primitives
 * using SETNX for leader election.
 *
 * <p>This implementation uses Redis for:
 *
 * <ul>
 *   <li>Leader election via SETNX (SET if Not eXists) with TTL
 *   <li>Membership tracking via Redis pub/sub
 *   <li>Distributed state storage via Redis hashes
 * </ul>
 *
 * <p>Thread Safety: This class is thread-safe. All mutable state is protected by volatile fields
...
public class RedisClusterManager implements ClusterManager {
  ...
  public void join(ClusterConfig config) throws ClusterJoinException {
    ...
    try {
      ...
      scheduler.scheduleAtFixedRate(this::tryBecomeLeader, 0, renewalPeriod, TimeUnit.SECONDS);
  ...
  private void tryBecomeLeader() {
    try (Jedis jedis = pool.getResource()) {
      ...
      String result = jedis.set(key, nodeId, SetParams.setParams().nx().ex(config.getLeaseTtl()));
