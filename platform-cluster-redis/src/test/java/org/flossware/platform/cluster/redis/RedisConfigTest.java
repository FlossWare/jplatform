...
class RedisConfigTest {
  ...
  @Test
  void testCustomHost() {
    RedisConfig config = RedisConfig.builder().host("redis.example.com").build();
  ...
  @Test
  void testCustomPort() {
    RedisConfig config = RedisConfig.builder().port(6380).build();
  ...
  void testCustomLeaseTtl() {
    RedisConfig config = RedisConfig.builder().leaseTtl(30).build();
  ...
  void testInvalidLeaseTtl() {
    assertThrows(IllegalArgumentException.class, () -> RedisConfig.builder().leaseTtl(4).build());
  }

  @Test
  void testPassword() {
    RedisConfig config = RedisConfig.builder().password("secret").build();
  ...
  @Test
  void testDatabase() {
    RedisConfig config = RedisConfig.builder().database(5).build();
  ...
  void testInvalidDatabase() {
    assertThrows(IllegalArgumentException.class, () -> RedisConfig.builder().database(-1).build());
  }

  @Test
  void testTimeout() {
    RedisConfig config = RedisConfig.builder().timeout(5000).build();
  ...
  void testInvalidTimeout() {
    assertThrows(IllegalArgumentException.class, () -> RedisConfig.builder().timeout(0).build());
  }
