...
class DatabaseStorageConfigTest {
  ...
  void testBuilderDefaults() {
    DatabaseStorageConfig config =
        DatabaseStorageConfig.builder().jdbcUrl("jdbc:h2:mem:test").build();
  ...
  void testEmptyJdbcUrl() {
    Exception exception =
        assertThrows(
            IllegalStateException.class,
            () -> {
              DatabaseStorageConfig.builder().jdbcUrl("  ").build();
            });
    assertTrue(exception.getMessage().contains("JDBC URL"));
  }
  ...
  void testMaxPoolSizeValidation() {
    Exception exception =
        assertThrows(
            ...
            () -> {
              DatabaseStorageConfig.builder().jdbcUrl("jdbc:h2:mem:test").maxPoolSize(0).build();
            });
  ...
  void testMaxPoolSizeNegative() {
    Exception exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              DatabaseStorageConfig.builder().jdbcUrl("jdbc:h2:mem:test").maxPoolSize(-5).build();
            });
    assertTrue(exception.getMessage().contains("Max pool size"));
  }
  ...
  void testH2Configuration() {
    DatabaseStorageConfig config =
        DatabaseStorageConfig.builder().jdbcUrl("jdbc:h2:mem:volumes").build();
