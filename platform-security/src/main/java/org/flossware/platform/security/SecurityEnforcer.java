 ...
 * operations and checking them against application security policies. It uses the StackWalker API
 * (Java 9+) to identify the calling ClassLoader and apply the appropriate security policy.
 *
 * <p><b>Advantages over SecurityManager:</b>
 *
 * <ul>
 *   <li>Not deprecated - works with Java 17+
 *   <li>Better performance - no global permission checks
 *   <li>More flexible - can be enabled/disabled per operation
 *   <li>Cleaner stack traces - no deep SecurityManager call chains
 * </ul>
 *
 * <p><b>Usage:</b>
 *
 ...
 *
 * <p>This enforcer uses StackWalker to identify the caller's ClassLoader and look up the
 * corresponding security policy. It provides manual enforcement points that applications can call,
 * or can be integrated with bytecode instrumentation for transparent enforcement.
 *
 * @since 2.0
 * @see SecurityPolicy
...
public final class SecurityEnforcer {
   ...
   *
   * <p>When disabled, all security checks are bypassed. This is useful for development/testing but
   ...
   *
   * <p>All classes loaded by this ClassLoader will be subject to the registered security policy.
   *
   ...
   *
   * <p>This method uses StackWalker to determine the calling class's ClassLoader and checks the
   ...
   *
   * <p>This walks the stack and finds the first class that is not in this security package, then
   * returns its ClassLoader.
   *
    ...
    return stackWalker.walk(
        frames -> {
          return frames
              ...
              .filter(frame -> !frame.getClassName().startsWith("org.flossware.platform.security"))
              .findFirst()
   ...
   *
   * <p>Trusted classloaders are:
   *
   * <ul>
   *   <li>Platform classloader - loads platform modules
   *   <li>System classloader - loads platform-java core classes
   * </ul>
   *
  ...
  private boolean isPlatformClassLoader(ClassLoader cl) {
    ...
    return cl == ClassLoader.getPlatformClassLoader() || cl == ClassLoader.getSystemClassLoader();
  }
   ...
   *
   * <p>This should only be used during platform shutdown or testing.
   */
