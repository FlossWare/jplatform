...
class SecurityEnforcerTest {
  ...
  void setUp() throws Exception {
    ...
    testClassLoader = new URLClassLoader(new URL[] {}, SecurityEnforcerTest.class.getClassLoader());
  ...
  void testCheckFileAccessWhenDisabled() {
    ...
    assertDoesNotThrow(() -> enforcer.checkFileAccess("/tmp/test.txt", "read"));
  }
  ...
  void testCheckSocketAccessWhenDisabled() {
    ...
    assertDoesNotThrow(() -> enforcer.checkSocketAccess("example.com", 80, "connect"));
  }
  ...
  void testCheckReflectionAccessWhenDisabled() {
    ...
    assertDoesNotThrow(() -> enforcer.checkReflectionAccess());
  }
  ...
  void testCheckNativeAccessWhenDisabled() {
    ...
    assertDoesNotThrow(() -> enforcer.checkNativeAccess("mylib"));
  }
  ...
  void testMultiplePolicies() throws Exception {
    URLClassLoader classLoader2 =
        new URLClassLoader(new URL[] {}, SecurityEnforcerTest.class.getClassLoader());
  ...
  private static class MockSecurityPolicy implements SecurityPolicy {
    ...
    public boolean checkPermission(Permission permission) {
      ...
      return granted.isEmpty() || granted.stream().anyMatch(p -> p.implies(permission));
    }
  ...
  void testEnforcePermissionNoPolicy() {
    ...
    try {
      customLoader =
          new URLClassLoader(
              new URL[] {}, null // Parent is null, so it won't be a platform classloader
              );
