...
/**
 * ZooKeeper-based configuration source. Loads configuration from ZooKeeper and supports dynamic
 * updates via watchers.
 *
 * <p>This implementation:
 *
 * <ul>
 *   <li>Stores configuration in ZooKeeper hierarchical znodes
 *   <li>Supports dynamic configuration updates via TreeCache
 *   <li>Maintains local cache for fast access
 *   <li>Automatically reconnects on connection loss
 * </ul>
 *
...
public class ZooKeeperConfigSource implements AutoCloseable {
  ...
  /**
   * Starts the ZooKeeper configuration source. Must be called before using the configuration
   * source.
   */
  public void start() {
    if (started) {
    ...
    try {
      client =
          CuratorFrameworkFactory.builder()
              ...
              .retryPolicy(
                  new ExponentialBackoffRetry(config.getRetryIntervalMs(), config.getRetryCount()))
              .build();
  ...
  private void ensureBasePath() throws Exception {
    if (client.checkExists().forPath(config.getBasePath()) == null) {
      client.create().creatingParentsIfNeeded().forPath(config.getBasePath());
    }
  }
  ...
  private void startWatching() throws Exception {
    treeCache = TreeCache.newBuilder(client, config.getBasePath()).setCacheData(true).build();
