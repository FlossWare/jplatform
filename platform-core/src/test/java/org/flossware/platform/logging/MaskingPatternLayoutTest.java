...
/**
 * Unit tests for MaskingPatternLayout. Verifies that sensitive data is properly redacted from log
 * messages.
 */
@Tag("unit")
@Tag("security")
class MaskingPatternLayoutTest {
  ...
  void testBearerTokenMasking() {
    ...
    assertFalse(
        result.contains("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"), "Bearer token should be masked");
    assertTrue(result.contains("***REDACTED***"), "Should contain redaction marker");
  }
  ...
  void testMultipleSecretsInSameMessage() {
    ...
    event.setLevel(Level.INFO);
    event.setMessage(
        "Credentials: {\"apiKey\": \"key123\", \"password\": \"pass456\", \"token\": \"tok789\"}");
  ...
  void testGetMaskPatterns() {
    ...
    assertEquals(2, layout.getMaskPatterns().size(), "Should have 2 patterns configured");
  }
  ...
  void testAccessTokenMasking() {
    ...
    event.setLevel(Level.INFO);
    event.setMessage(
        "OAuth response: {\"access_token\": \"ya29.a0AfH6SMB...\", \"expires_in\": 3600}");
  ...
  void testClientSecretMasking() {
    ...
    event.setLevel(Level.INFO);
    event.setMessage(
        "OAuth config: {\"client_id\": \"app123\", \"client_secret\": \"cs_secret456\"}");
