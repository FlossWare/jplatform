 ...
 *
 * <ul>
 *   <li>Resolves native executable from descriptor classpath entries
 *   <li>Sets up process environment variables
 *   <li>Launches process via ProcessBuilder
 *   <li>Redirects stdout/stderr to logging
 *   <li>Monitors process lifecycle
 * </ul>
 *
 * <p>Native applications are launched as separate OS processes, not JVM applications. This enables
 * deployment of GraalVM native images and other compiled executables.
 *
 * @since 2.0
 */
public class NativeProcessLauncher {
   ...
   */
  public Process launch(String applicationId, ApplicationDescriptor descriptor, Path workingDir)
      throws IOException {
   ...
   * @throws SecurityException if the path contains path traversal attempts or points to restricted
   *     directories
   */
  ...
  private void validateExecutablePath(String path) {
    ...
    if (path.contains("../") || path.contains("..\\")) {
      throw new SecurityException(
          "Executable path contains illegal path traversal sequence: " + path);
    }
    ...
    for (String restrictedDir : RESTRICTED_DIRECTORIES) {
      if (normalizedPathStr.startsWith(restrictedDir + "/")
          || normalizedPathStr.equals(restrictedDir)) {
        throw new SecurityException(
            "Executable path points to restricted system directory: " + path);
      }
    }
