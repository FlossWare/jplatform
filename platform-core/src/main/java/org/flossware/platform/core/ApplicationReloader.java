 ...
 *
 * <ul>
 *   <li>Creates new classloader with updated JARs
 *   <li>Captures state from old application instance (if ReloadableApplication)
 *   <li>Swaps classloader references atomically
 *   <li>Starts new application instance with restored state
 *   <li>Tracks classloader versions for potential rollback
 * </ul>
 *
 * <p>Reload process:
 *
 * <ol>
 *   <li>Validate application is in RUNNING or STOPPED state
 *   <li>Create new IsolatedClassLoader with updated descriptor
 *   <li>If ReloadableApplication, call beforeReload() to capture state
 *   <li>Stop old application instance
 *   <li>Atomically swap classloader in ApplicationContext
 *   <li>Load main class from new classloader
 *   <li>Create new application instance
 *   <li>If ReloadableApplication, call afterReload() with saved state
 *   <li>Start new application instance
 *   <li>Keep old classloader for potential rollback
 * </ol>
 *
 * @since 2.0
 */
public class ApplicationReloader {
   ...
   *
   * <p>This method handles the complete reload process including state preservation, classloader
   * swapping, and instance recreation.
   *
  ...
  public synchronized void reload(
    ...
    try {
      ...
      newClassLoader =
          IsolatedClassLoader.create(applicationId, newDescriptor, platformSharedLoader);
      ...
      try {
        ...
        throw new Exception(
            "Main class " + mainClassName + " must have a public no-arg constructor for hot reload",
            e);
      }
   ...
   *
   * <p>This allows old classloaders to be garbage collected if no longer referenced. Keeps a few
   * recent versions for potential rollback.
   *
   * @param applicationId the application identifier
  ...
  /**
   * Clears version history for an application. Called when application is undeployed.
   *
   * @param applicationId the application identifier
   */
