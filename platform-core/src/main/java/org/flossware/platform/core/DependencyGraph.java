 ...
 *
 * <ul>
 *   <li>Adding dependency edges between applications
 *   <li>Detecting circular dependencies
 *   <li>Topological sorting for startup order
 * </ul>
 *
 * <p>Example usage:
 *
...
class DependencyGraph {
  ...
  private List<String> detectCycleDFS(
      String node, Set<String> visited, Set<String> recursionStack, Map<String, String> parent) {
    visited.add(node);
   ...
   * @throws IllegalStateException if parent chain is broken or cycle reconstruction loops
   *     excessively
   */
   ...
   *
   * <p>Returns applications in an order such that all dependencies of an application appear before
    ...
    if (result.size() != adjacencyList.size()) {
      throw new IllegalStateException(
          "Topological sort incomplete: "
              ...
              + " of "
              + adjacencyList.size()
              + " nodes processed. This indicates a cycle that was not detected.");
    }
