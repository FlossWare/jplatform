 ...
 *
 * <p>Automatically selects the optimal execution backend (in-JVM, container, VM, Kubernetes) for
 * each workload based on characteristics and requirements. Similar to VirtOS IaaS scheduling, users
 * describe <b>what</b> they want, and the scheduler decides <b>how</b> to run it.
 *
 * <p><b>Decision Factors:</b>
 *
 * <ul>
 *   <li><b>Workload Type</b> - Java app, native binary, container image
 *   <li><b>Resource Requirements</b> - CPU, memory, disk needs
 *   <li><b>Isolation Needs</b> - Kernel access, security policies
 *   <li><b>Scale Requirements</b> - Replica count, auto-scaling
 *   <li><b>Infrastructure Availability</b> - Which backends are available
 * </ul>
 *
 * <p><b>Decision Tree:</b>
 *
...
public class WorkloadPlacementScheduler {
   ...
   *
   * <p>Analyzes the application descriptor and selects the best backend based on workload
   * characteristics and infrastructure availability.
   *
  ...
  private PlacementDecision applyDecisionTree(WorkloadProfile profile) {
    ...
    if (profile.requiresVmIsolation()) {
      return new PlacementDecision(
          appId, ExecutionBackend.VIRTUAL_MACHINE, "VM isolation explicitly required via policy");
    }

    // Rule 2: Kernel access required
    if (profile.needsKernelAccess()) {
      return new PlacementDecision(
          appId, ExecutionBackend.VIRTUAL_MACHINE, "Workload requires kernel-level access");
    }
    ...
    if (profile.requiresMassiveScale() && kubernetesAvailable) {
      return new PlacementDecision(
          ...
          String.format(
              "Massive scale deployment (%d replicas) requires Kubernetes",
    ...
    if (!profile.isJavaApp()) {
      if (profile.isContainerImage()) {
        return new PlacementDecision(
            appId, ExecutionBackend.CONTAINER, "Non-Java workload with container image specified");
        ...
        return new PlacementDecision(
            appId, ExecutionBackend.CONTAINER, "Non-Java workload defaults to container");
      }
    }
    ...
    if (profile.isJavaApp() && profile.getMemoryMB() >= HEAVY_MEMORY_THRESHOLD_MB) {
      return new PlacementDecision(
          ...
          String.format(
              "Heavy Java workload (%dMB memory) requires container isolation",
              profile.getMemoryMB()));
    }
    ...
    if (profile.isJavaApp() && profile.getMemoryMB() >= LIGHTWEIGHT_MEMORY_THRESHOLD_MB) {
      return new PlacementDecision(
          ...
          ExecutionBackend.CONTAINER,
          String.format(
              "Moderate Java workload (%dMB memory) benefits from container isolation",
              profile.getMemoryMB()));
    }
    ...
    if (profile.isJavaApp() && profile.getMemoryMB() < LIGHTWEIGHT_MEMORY_THRESHOLD_MB) {
      return new PlacementDecision(
          ...
          String.format(
              "Lightweight Java workload (%dMB memory, %d CPUs) optimal for in-JVM execution",
    ...
    return new PlacementDecision(
        appId, ExecutionBackend.CONTAINER, "Default fallback to container execution");
  }
