...
class RedisStorageConfigTest {
  ...
  void testBuilderDefaults() {
    RedisStorageConfig config = RedisStorageConfig.builder().build();
  ...
  void testMissingHost() {
    Exception exception =
        assertThrows(
            ...
            () -> {
              RedisStorageConfig.builder().host(null).build();
            });
  ...
  void testEmptyHost() {
    Exception exception =
        assertThrows(
            IllegalStateException.class,
            () -> {
              RedisStorageConfig.builder().host("  ").build();
            });
    assertTrue(exception.getMessage().contains("Host"));
  }
  ...
  void testPortTooSmall() {
    Exception exception =
        assertThrows(
            ...
            () -> {
              RedisStorageConfig.builder().port(0).build();
            });
  ...
  void testPortTooLarge() {
    Exception exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              RedisStorageConfig.builder().port(65536).build();
            });
    assertTrue(exception.getMessage().contains("Port"));
  }
  ...
  void testDatabaseNegative() {
    Exception exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              RedisStorageConfig.builder().database(-1).build();
            });
  ...
  void testConnectionTimeoutTooSmall() {
    Exception exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              RedisStorageConfig.builder().connectionTimeout(50).build();
            });
  ...
  void testSocketTimeoutTooSmall() {
    Exception exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              RedisStorageConfig.builder().socketTimeout(99).build();
            });
  ...
  void testAuthConfiguration() {
    RedisStorageConfig config =
        RedisStorageConfig.builder().host("redis.example.com").password("secure-password").build();
  ...
  void testMultipleDatabases() {
    RedisStorageConfig db0 = RedisStorageConfig.builder().database(0).build();

    RedisStorageConfig db1 = RedisStorageConfig.builder().database(1).build();
  ...
  void testCustomKeyPrefix() {
    RedisStorageConfig config = RedisStorageConfig.builder().keyPrefix("prod:volumes:").build();
  ...
  void testMinimumTimeouts() {
    RedisStorageConfig config =
        RedisStorageConfig.builder().connectionTimeout(100).socketTimeout(100).build();
