...
public class ClassLoaderStatistics {
  ...
  public ClassLoaderStatistics(
      String applicationId, int classesLoaded, long totalBytesLoaded, long cacheHits) {
    ...
    if (totalBytesLoaded < 0) {
      throw new IllegalArgumentException(
          "totalBytesLoaded cannot be negative: " + totalBytesLoaded);
    }
    if (cacheHits < 0) {
  ...
  public String toString() {
    return String.format(
        "ClassLoaderStatistics{app=%s, classes=%d, bytes=%d, cacheHits=%d, hitRate=%.2f%%}",
