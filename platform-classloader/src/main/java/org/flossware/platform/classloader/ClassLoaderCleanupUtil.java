 ...
 *
 * <p>ClassLoader leaks are a common problem in Java platforms that load and unload applications
 * dynamically. This utility provides comprehensive cleanup for common leak sources:
 *
 * <ul>
 *   <li>ThreadLocal variables
 *   <li>JDBC drivers registered with DriverManager
 *   <li>JMX MBeans
 *   <li>Shutdown hooks
 *   <li>Resource bundle caches
 * </ul>
 *
 * <p>Usage:
 *
...
public class ClassLoaderCleanupUtil {
  ...
  /**
   * Checks if a ClassLoader is the target ClassLoader or a descendant of it. Handles
   * wrapped/proxied ClassLoaders and delegation hierarchies.
   *
   ...
   *
   * <p>This is the main entry point for ClassLoader cleanup. It calls all individual cleanup
   ...
   *
   * <p>ThreadLocals are a common source of ClassLoader leaks. This method clears ThreadLocals from
   * all threads whose names contain the application ID.
   */
   ...
   *
   * <p>JDBC drivers registered with DriverManager create a permanent reference to the ClassLoader
   * that loaded them. This method deregisters all drivers loaded by our ClassLoader.
   */
   ...
   *
   * <p>MBeans registered by applications can hold references to the ClassLoader. This method
   * unregisters all MBeans whose ObjectName contains the application ID.
   */
   ...
   *
   * <p>Applications may register shutdown hooks that hold references to the ClassLoader. This
   * method attempts to remove hooks for threads whose names contain the application ID.
   */
   ...
   *
   * <p>ResourceBundles are cached by ClassLoader, which can prevent garbage collection. This method
   * clears the cache using reflection.
   */
   ...
   *
   * <p>This method should be called after {@link #cleanupAll()} to verify that the ClassLoader is
   * eligible for garbage collection. It triggers a GC and checks if the WeakReference has been
   * cleared.
   *
