...
/**
 * Utility class for formatting metrics in Prometheus text exposition format. Provides static
 * methods to convert metrics into the Prometheus text format with proper label escaping.
 *
 * <p>The Prometheus text format consists of:
 *
 * <ul>
 *   <li>HELP lines describing the metric
 *   <li>TYPE lines declaring the metric type (gauge, counter, etc.)
 *   <li>Metric lines with labels and values
 * </ul>
 *
 * <p>Example output:
 *
...
public final class PrometheusFormatter {
  ...
  /**
   * Formats a HELP line describing a metric. HELP lines provide human-readable documentation for
   * metrics.
   *
  ...
  /**
   * Formats a TYPE line declaring a metric type. Common types are: counter, gauge, histogram,
   * summary.
   *
   * @param name the metric name
  ...
  /**
   * Formats a gauge metric with labels and value. Gauges represent values that can go up or down
   * (e.g., memory usage, active connections).
   *
   * @param name the metric name
  ...
  /**
   * Formats a counter metric with labels and value. Counters represent cumulative values that only
   * increase (e.g., total requests, CPU time).
   *
   * @param name the metric name
   * @param labels the label key-value pairs (may be empty or null)
  ...
  /**
   * Formats label key-value pairs with proper escaping. Labels are formatted as:
  ...
  /**
   * Escapes special characters in label values according to Prometheus format. Escapes backslashes,
   * double quotes, newlines, carriage returns, and tabs.
   *
  ...
  /**
   * Formats a numeric value for Prometheus. Special handling for NaN and infinity values.
   *
