...
/**
 * Collects metrics from an ApplicationContext and formats them for Prometheus export. Converts
 * platform metrics (CPU, memory, threads, state) into Prometheus text format.
 *
 * <p>Collected metrics include:
 *
 * <ul>
 *   <li><b>jplatform_app_cpu_time_seconds</b> (counter) - Total CPU time consumed
 *   <li><b>jplatform_app_heap_used_bytes</b> (gauge) - Current heap memory usage
 *   <li><b>jplatform_app_thread_count</b> (gauge) - Number of active threads
 *   <li><b>jplatform_app_state</b> (gauge) - Application state (1 for current state, 0 for others)
 *   <li><b>jplatform_app_threadpool_active</b> (gauge) - Active thread pool threads
 *   <li><b>jplatform_app_threadpool_queued</b> (gauge) - Queued tasks in thread pool
 *   <li><b>jplatform_app_threadpool_completed</b> (counter) - Total completed tasks
 * </ul>
 *
 * <p>All metrics include an {@code app_id} label identifying the application.
 *
 * <p>Example output:
 *
 * <pre>{@code
...
public final class ApplicationMetricsCollector {
  ...
  public static String collectMetrics(ApplicationContext context) {
    try {
      ...
      return "# ERROR: Failed to collect metrics: "
          + e.getMessage()
          + "\n"
          + "jplatform_metrics_collection_errors_total{app_id=\""
          + appId
          + "\"} 1\n";
    }
  }
  ...
  private static void collectResourceMetrics(
    ...
    sb.append(
        PrometheusFormatter.formatHelp(
            "jplatform_app_cpu_time_seconds", "Total CPU time used by application"));
    ...
    sb.append(
        PrometheusFormatter.formatCounter("jplatform_app_cpu_time_seconds", labels, cpuSeconds));

    // Heap memory usage (gauge)
    sb.append(
        PrometheusFormatter.formatHelp(
            "jplatform_app_heap_used_bytes", "Heap memory used by application"));
    sb.append(PrometheusFormatter.formatType("jplatform_app_heap_used_bytes", "gauge"));
    sb.append(
        PrometheusFormatter.formatGauge(
            "jplatform_app_heap_used_bytes", labels, snapshot.getHeapUsedBytes()));

    // Thread count (gauge)
    sb.append(
        PrometheusFormatter.formatHelp(
            "jplatform_app_thread_count", "Number of active threads in application"));
    sb.append(PrometheusFormatter.formatType("jplatform_app_thread_count", "gauge"));
    sb.append(
        PrometheusFormatter.formatGauge(
            "jplatform_app_thread_count", labels, snapshot.getThreadCount()));
  }
  ...
  private static void collectThreadPoolMetrics(
    ...
    sb.append(
        PrometheusFormatter.formatHelp(
            "jplatform_app_threadpool_active", "Number of threads actively executing tasks"));
    sb.append(PrometheusFormatter.formatType("jplatform_app_threadpool_active", "gauge"));
    sb.append(
        PrometheusFormatter.formatGauge(
            "jplatform_app_threadpool_active", labels, stats.getActiveThreads()));

    // Queued tasks (gauge)
    sb.append(
        PrometheusFormatter.formatHelp(
            "jplatform_app_threadpool_queued", "Number of tasks waiting in thread pool queue"));
    sb.append(PrometheusFormatter.formatType("jplatform_app_threadpool_queued", "gauge"));
    sb.append(
        PrometheusFormatter.formatGauge(
            "jplatform_app_threadpool_queued", labels, stats.getQueuedTasks()));

    // Completed tasks (counter)
    sb.append(
        PrometheusFormatter.formatHelp(
            "jplatform_app_threadpool_completed", "Total number of completed tasks"));
    sb.append(PrometheusFormatter.formatType("jplatform_app_threadpool_completed", "counter"));
    sb.append(
        PrometheusFormatter.formatCounter(
            "jplatform_app_threadpool_completed", labels, stats.getCompletedTasks()));
  }

  /**
   * Collects application state metrics. Creates a gauge metric for each possible state, with value
   * 1 for current state and 0 for others.
   *
   * @param sb the StringBuilder to append metrics to
