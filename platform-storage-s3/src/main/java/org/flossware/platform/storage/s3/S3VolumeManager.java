...
/**
 * S3-based implementation of VolumeManager. Provides persistent storage using AWS S3 or
 * S3-compatible services like MinIO.
 *
 * <p>This implementation:
 *
 * <ul>
 *   <li>Maps each volume to a prefix in an S3 bucket
 *   <li>Maintains a local cache directory for file operations
 *   <li>Synchronizes files between local cache and S3
 *   <li>Tracks object sizes for usage calculations
 * </ul>
 *
...
public class S3VolumeManager implements VolumeManager, AutoCloseable {
  ...
  /**
   * Initializes the S3 client and creates the bucket if needed. Must be called before using the
   * volume manager.
   */
  public void start() {
    if (initialized) {
  ...
  public Path getVolumePath(String volumeName) {
    ...
    return localCachePaths.computeIfAbsent(
        volumeName,
        name -> {
          try {
            Path cachePath =
                Paths.get(System.getProperty("java.io.tmpdir"), "s3-volumes", volumeName);
  ...
  public long getVolumeUsageBytes(String volumeName) throws IOException {
    ...
    try {
      ...
      ListObjectsV2Request request =
          ListObjectsV2Request.builder().bucket(config.getBucketName()).prefix(prefix).build();
