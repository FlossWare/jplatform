  ...
  void testBuilderMissingAccessKey() {
    Exception exception =
        assertThrows(
            ...
            () -> {
              S3StorageConfig.builder().secretKey("secret").bucketName("bucket").build();
            });
  ...
  void testBuilderMissingSecretKey() {
    Exception exception =
        assertThrows(
            IllegalStateException.class,
            () -> {
              S3StorageConfig.builder().accessKey("key").bucketName("bucket").build();
            });
  ...
  void testBuilderEmptySecretKey() {
    Exception exception =
        assertThrows(
            IllegalStateException.class,
            () -> {
              S3StorageConfig.builder().accessKey("key").secretKey("").bucketName("bucket").build();
            });
    assertTrue(exception.getMessage().contains("Secret key"));
  }
  ...
  void testBuilderMissingBucketName() {
    Exception exception =
        assertThrows(
            IllegalStateException.class,
            () -> {
              S3StorageConfig.builder().accessKey("key").secretKey("secret").build();
            });
