 ...
 * logic and error handling, delegating format-specific ObjectMapper creation to subclasses.
 *
 * <p>Subclasses must implement {@link #createObjectMapper()} to provide the appropriate
 * ObjectMapper for their format (YAML, JSON, etc.).
 *
 * <p>This class handles:
 *
 * <ul>
 *   <li>Common validation logic
 *   <li>Exception handling and wrapping in ParseException
 *   <li>File I/O using {@link #parseFile(Path)}
 *   <li>Stream-based parsing using {@link #parse(InputStream)}
 * </ul>
 *
...
public abstract class AbstractDescriptorParser implements ApplicationDescriptorParser {
  ...
  /**
   * Creates the ObjectMapper for parsing the specific format. Subclasses must implement this to
   * provide the appropriate mapper (e.g., YAMLMapper for YAML, standard ObjectMapper for JSON).
   *
  ...
  /**
   * Returns the ObjectMapper used by this parser. Exposed for testing purposes.
   *
