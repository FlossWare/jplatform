...
class ZookeeperConfigTest {
  ...
  void testCustomConnectionString() {
    ZookeeperConfig config =
        ZookeeperConfig.builder().connectionString("zk1:2181,zk2:2181,zk3:2181").build();
  ...
  void testCustomSessionTimeout() {
    ZookeeperConfig config = ZookeeperConfig.builder().sessionTimeoutMs(60000).build();
  ...
  void testCustomConnectionTimeout() {
    ZookeeperConfig config = ZookeeperConfig.builder().connectionTimeoutMs(30000).build();
  ...
  void testCustomBaseSleepTime() {
    ZookeeperConfig config = ZookeeperConfig.builder().baseSleepTimeMs(2000).build();
  ...
  void testInvalidBaseSleepTime() {
    assertThrows(
        IllegalArgumentException.class, () -> ZookeeperConfig.builder().baseSleepTimeMs(0).build());
  }

  @Test
  void testCustomMaxRetries() {
    ZookeeperConfig config = ZookeeperConfig.builder().maxRetries(5).build();
  ...
  void testInvalidMaxRetries() {
    assertThrows(
        IllegalArgumentException.class, () -> ZookeeperConfig.builder().maxRetries(-1).build());
  }

  @Test
  void testNamespace() {
    ZookeeperConfig config = ZookeeperConfig.builder().namespace("jplatform").build();
  ...
  void testEmptyConnectionString() {
    assertThrows(
        IllegalStateException.class, () -> ZookeeperConfig.builder().connectionString("").build());
  }
  ...
  void testZeroMaxRetries() {
    ZookeeperConfig config = ZookeeperConfig.builder().maxRetries(0).build();
  ...
  void testEmptyNamespace() {
    ZookeeperConfig config = ZookeeperConfig.builder().namespace("").build();
  ...
  void testBuilderReuse() {
    ZookeeperConfig.Builder builder = ZookeeperConfig.builder().connectionString("host1:2181");

    ZookeeperConfig config1 = builder.build();
    ZookeeperConfig config2 = builder.sessionTimeoutMs(45000).build();
  ...
  void testLargeMaxRetries() {
    ZookeeperConfig config = ZookeeperConfig.builder().maxRetries(Integer.MAX_VALUE).build();
