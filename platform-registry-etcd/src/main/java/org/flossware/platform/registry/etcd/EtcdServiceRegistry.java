 ...
 *
 * <ul>
 *   <li>Stores service metadata in etcd KV store
 *   <li>Uses leases for automatic cleanup of dead services
...
public class EtcdServiceRegistry implements ServiceRegistry, AutoCloseable {
  ...
  /** Initializes the etcd client connection. Must be called before using the registry. */
  public void start() {
  ...
  public <T> void registerService(Class<T> serviceInterface, T implementation) {
    ...
    try {
      ...
      LeaseGrantResponse leaseResp =
          leaseClient.grant(config.getLeaseTtl()).get(5, TimeUnit.SECONDS);
    ...
    for (Object service : services) {
      if (!serviceInterface.isInstance(service)) {
        ...
        throw new IllegalStateException(
            "Service registry corrupted: service "
                + service.getClass().getName()
                + " does not implement "
                + serviceInterface.getName());
      }
