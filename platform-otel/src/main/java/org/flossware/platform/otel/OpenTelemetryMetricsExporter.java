 ...
 *
 * <p>Exports application metrics to OpenTelemetry Collector using OTLP protocol. Metrics include:
 *
 * <ul>
 *   <li>CPU time (counter)
 *   <li>Heap usage (gauge)
 *   <li>Thread count (gauge)
 *   <li>Application state (gauge)
 * </ul>
 *
 * <p>All metrics include an {@code app_id} attribute for filtering.
 *
 * <p>Configuration:
 *
 * <pre>
...
public class OpenTelemetryMetricsExporter implements MetricsExporter {
   ...
   *
   * <p>Uses environment variables for OTLP endpoint configuration:
   *
   * <ul>
   *   <li>OTEL_EXPORTER_OTLP_ENDPOINT (default: http://localhost:4317)
   *   <li>OTEL_EXPORTER_OTLP_PROTOCOL (default: grpc)
   * </ul>
   */
  ...
  public OpenTelemetryMetricsExporter(String otlpEndpoint) {
    ...
    OtlpGrpcMetricExporter otlpExporter =
    ...
    PeriodicMetricReader metricReader =
        PeriodicMetricReader.builder(otlpExporter).setInterval(Duration.ofSeconds(60)).build();
    ...
    Resource resource =
        Resource.getDefault()
            .merge(
                Resource.create(
                    Attributes.of(AttributeKey.stringKey("service.name"), "jplatform")));
  ...
  public void registerApplication(String applicationId, ApplicationContext context) {
    ...
    // CPU time counter
    appInstruments.cpuTimeCounter =
        meter
            .counterBuilder("platform.app.cpu_time_seconds")
            .setDescription("Total CPU time consumed by the application")
            .setUnit("s")
            .build();

    // Heap usage gauge
    appInstruments.heapGauge =
        meter
            .gaugeBuilder("platform.app.heap_used_bytes")
            .ofLongs()
    ...
    // Thread count gauge
    appInstruments.threadGauge =
        meter
            .gaugeBuilder("platform.app.thread_count")
            .ofLongs()
  ...
  /**
   * Exports metrics for an application. Note: This is called periodically by the monitoring system,
   * not by MetricsExporter interface.
   *
   ...
   *
   * <p>Flushes any pending metrics and releases resources.
   */
