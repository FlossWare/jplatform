 ...
 *
 * <p>Note: These tests require a running libvirt daemon and are disabled by default. To run them,
 * set system property: -Dlibvirt.available=true
 */
class VmLauncherTest {
  ...
  void testLaunchFailsWithoutDisk() throws Exception {
    ...
    try {
      ...
      assertTrue(
          exception.getMessage().contains("vm.disk"),
      ...
      org.junit.jupiter.api.Assumptions.assumeTrue(
          false, "Skipping test - libvirt not available: " + e.getMessage());
    }
  }
  ...
  void testDefaultValues() {
    ...
    assertEquals(
        "/var/lib/jplatform/vms/default-test.qcow2", descriptor.getProperties().get("vm.disk"));
  ...
  void testVmStatsClass() {
    VmLauncher.VmStats stats =
        new VmLauncher.VmStats(
            8192, // memoryMB
            16384, // maxMemoryMB
            4, // vcpu
  ...
  void testSnapshotOperations() throws Exception {
    ...
    try {
      ...
      // Create snapshot
      String snapshotName =
          launcher.createSnapshot("snapshot-test-vm", vmInfo, "test-snapshot", "Test snapshot");
  ...
  /**
   * Tests live migration (requires two libvirt hosts). Note: This test is informational only -
  ...
  void testLiveMigration() throws Exception {
    ...
    try {
      ...
      String destUri = System.getProperty("libvirt.migration.dest.uri", "qemu+ssh://host2/system");
  ...
  void testVmStatsDifferentStates() {
    ...
    for (String state : states) {
      VmLauncher.VmStats stats = new VmLauncher.VmStats(4096, 8192, 2, 1000000L, state);
  ...
  void testVmConfigurationMultipleDisks() {
    ...
    assertEquals("/var/lib/jplatform/vms/os.qcow2", descriptor.getProperties().get("vm.disk"));
    assertEquals("/var/lib/jplatform/vms/data1.qcow2", descriptor.getProperties().get("vm.disk.1"));
    assertEquals("/var/lib/jplatform/vms/data2.qcow2", descriptor.getProperties().get("vm.disk.2"));
  }
  ...
  void testVmStatsCpuTimePrecision() {
    ...
    VmLauncher.VmStats stats = new VmLauncher.VmStats(2048, 4096, 2, 500000000L, "RUNNING");
  ...
  void testVmStatsNegativeCpuTime() {
    ...
    VmLauncher.VmStats stats = new VmLauncher.VmStats(2048, 4096, 2, -1000000000L, "RUNNING");
