 ...
 * providing unified orchestration across all workload types.
 *
 * <p>VM Configuration via Properties:
 *
 * <pre>
...
public class VmLauncher {
   ...
   */
  public VmInfo launch(String applicationId, ApplicationDescriptor descriptor)
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "vm.vcpu must be a valid integer, got: " + properties.get("vm.vcpu"), e);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "vm.memory must be a valid integer, got: " + properties.get("vm.memory"), e);
    }
  ...
  /**
   * Live migrate VM to another host. Migrates a running VM to another libvirt host with minimal
   * downtime.
   *
   * @param applicationId the application identifier
   * @param vmInfo the VM information
  ...
  /**
   * Create a snapshot of the VM. Captures the current state of the VM including memory and disk.
   *
   * @param applicationId the application identifier
   * @param vmInfo the VM information
  ...
  /**
   * Revert VM to a snapshot. Restores the VM to the state captured in the snapshot.
   *
   * @param applicationId the application identifier
   * @param vmInfo the VM information
  ...
  /**
   * Hot-add vCPUs to a running VM. Dynamically increases the number of CPUs without stopping the
   * VM.
   *
   * @param applicationId the application identifier
   * @param vmInfo the VM information
  ...
  /**
   * Hot-add memory to a running VM. Dynamically increases memory without stopping the VM.
   *
   * @param applicationId the application identifier
   * @param vmInfo the VM information
  ...
  /**
   * Resize VM resources (CPU and/or memory). Convenience method to adjust multiple resources at
   * once.
   *
   * @param applicationId the application identifier
   * @param vmInfo the VM information
  ...
  private String buildVmXml(
    ...
    xml.append("      <driver name='qemu' type='")
        .append(diskFormat)
        .append("' cache='writeback'/>\n");
    ...
    if ("true".equals(properties.get("vm.vnc.enabled"))) {
      ...
      xml.append("    <graphics type='vnc' port='")
          .append(vncPort)
          .append("' autoport='yes' listen='0.0.0.0'>\n");
