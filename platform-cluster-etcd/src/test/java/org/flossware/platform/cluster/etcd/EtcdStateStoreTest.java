...
class EtcdStateStoreTest {
  ...
  void testConstructorNullClientThrowsException() {
    assertThrows(IllegalArgumentException.class, () -> new EtcdStateStore(null));
  }
  ...
  void testPutApplicationState() throws Exception {
    ...
    when(mockKv.put(any(ByteSequence.class), any(ByteSequence.class))).thenReturn(putFuture);
  ...
  void testGetAllApplicationStates() throws Exception {
    ...
    when(kv1.getKey())
        .thenReturn(ByteSequence.from("/jplatform/states/app1", StandardCharsets.UTF_8));
    when(kv1.getValue()).thenReturn(ByteSequence.from(mapper.writeValueAsBytes(state1)));

    KeyValue kv2 = mock(KeyValue.class);
    when(kv2.getKey())
        .thenReturn(ByteSequence.from("/jplatform/states/app2", StandardCharsets.UTF_8));
  ...
  void testGetAllApplicationStatesException() throws Exception {
    ...
    assertThrows(RuntimeException.class, () -> store.getAllApplicationStates());
  }
  ...
  void testGetAllApplicationDescriptorsDeserializationError() throws Exception {
    KeyValue kv = mock(KeyValue.class);
    when(kv.getKey())
        .thenReturn(ByteSequence.from("/jplatform/descriptors/app1", StandardCharsets.UTF_8));
    when(kv.getValue()).thenReturn(ByteSequence.from("invalid json", StandardCharsets.UTF_8));
    ...
    assertThrows(RuntimeException.class, () -> store.getAllApplicationDescriptors());
  }
  ...
  void testListenerNotificationException() throws Exception {
    ...
    doThrow(new RuntimeException("Listener error"))
        .when(listener)
        .onStateChanged(anyString(), any(ApplicationState.class));
  ...
  void testClear() throws Exception {
    ...
    when(mockKv.delete(any(ByteSequence.class), any(DeleteOption.class))).thenReturn(deleteFuture);
