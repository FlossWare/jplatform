...
/**
 * Configuration for etcd-based clustering. Provides connection settings for etcd cluster and lease
 * parameters.
 *
 * <p>Example usage:
 *
...
public class EtcdConfig {
  ...
  /**
   * Returns the lease TTL in seconds. Leases are used for leader election and will expire if not
   * renewed.
   *
  ...
  /** Builder for EtcdConfig instances. Provides a fluent API for configuration. */
  public static class Builder {
     ...
     *
     * <p>The default endpoint "http://localhost:2379" is automatically removed when adding the
     * first non-localhost endpoint. If explicitly adding localhost, it is preserved.
     *
      ...
      for (String endpoint : endpoints) {
        if (endpoint == null || endpoint.trim().isEmpty()) {
          throw new IllegalArgumentException(
              "Endpoints list must not contain null or empty entries");
        }
      }
      this.endpoints.clear();
    ...
    public Builder leaseTtl(long leaseTtl) {
      ...
      if (leaseTtl > 86400) { // 24 hours
        throw new IllegalArgumentException(
            "Lease TTL too large (max 86400 seconds / 24 hours): " + leaseTtl);
      }
