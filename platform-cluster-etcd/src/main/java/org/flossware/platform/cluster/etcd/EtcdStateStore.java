 ...
 * descriptors using etcd key-value store.
 *
 * <p>This implementation uses:
 *
 * <ul>
 *   <li>etcd KV for storing application states
 *   <li>etcd KV for storing application descriptors
 *   <li>JSON serialization for complex objects
 *   <li>Key prefixes for data organization
 * </ul>
 *
...
public class EtcdStateStore implements ClusterStateStore {
  ...
  public Map<String, ApplicationState> getAllApplicationStates() {
    ...
    try {
      ...
      GetResponse response =
          kvClient
              .get(prefix, io.etcd.jetcd.options.GetOption.newBuilder().isPrefix(true).build())
              .get();
  ...
  /** Clears all stored state and descriptors. Useful for testing. */
  public void clear() {
    try {
      KV kvClient = client.getKVClient();
      ...
      kvClient
          .delete(
              statePrefix, io.etcd.jetcd.options.DeleteOption.newBuilder().isPrefix(true).build())
          .get();
