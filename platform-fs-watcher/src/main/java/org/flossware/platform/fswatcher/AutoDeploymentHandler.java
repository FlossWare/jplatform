...
/**
 * Automatic deployment listener that handles descriptor file events by deploying, redeploying, and
 * undeploying applications via ApplicationManager.
 *
 * <p>This listener integrates the filesystem watcher with the application management system. When
 * descriptor files are detected, modified, or removed, it automatically performs the appropriate
 * application lifecycle operations:
 *
 * <ul>
 *   <li><b>onDescriptorDetected:</b> Parse → Deploy → Optionally Start
 *   <li><b>onDescriptorModified:</b> Stop → Undeploy → Redeploy → Start
 *   <li><b>onDescriptorRemoved:</b> Stop → Undeploy
 *   <li><b>onError:</b> Log error details
 * </ul>
 *
 * <p>Example usage:
 *
 ...
 * }</pre>
 *
 * <p>The handler maintains a registry of which descriptor files correspond to which application
 * IDs, enabling proper tracking for redeploy and undeploy operations.
 *
...
public class AutoDeploymentHandler implements DeploymentEventListener {
  ...
  /**
   * Returns the descriptor registry used by this handler. Useful for testing and diagnostics.
   *
