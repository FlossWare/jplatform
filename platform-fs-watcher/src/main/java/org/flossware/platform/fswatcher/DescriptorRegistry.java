...
/**
 * Thread-safe registry that tracks the mapping between descriptor file paths and their deployed
 * application IDs.
 *
 * <p>This registry is used by the auto-deployment system to maintain state about which descriptor
 * files have been deployed and what application ID they correspond to. This is essential for
 * handling modify and remove events, where the system needs to know which application to redeploy
 * or undeploy.
 *
 * <p>Example usage:
 *
...
public class DescriptorRegistry {
  ...
  /**
   * Associates a descriptor file path with an application ID. If the path was already registered,
   * the previous application ID is replaced.
   *
  ...
  /**
   * Returns a copy of all registered mappings. Changes to the returned map will not affect the
   * registry.
   *
