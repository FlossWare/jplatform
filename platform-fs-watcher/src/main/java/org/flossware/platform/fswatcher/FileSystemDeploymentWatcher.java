...
package org.flossware.platform.fswatcher;
...
/**
 * Filesystem-based implementation of DeploymentWatcher that monitors a directory for application
 * descriptor files and triggers deployment events.
 *
 * <p>This implementation uses Java NIO's WatchService to efficiently monitor filesystem changes. It
 * includes the following features:
 *
 * <ul>
 *   <li>Thread-safe listener management using CopyOnWriteArrayList
 *   <li>Debouncing of file changes to avoid processing rapid modifications
 *   <li>File extension filtering based on WatcherConfig
 *   <li>Dedicated thread for watching via ExecutorService
 *   <li>Proper lifecycle management (start/stop/close)
 * </ul>
 *
 * <p>Example usage:
 *
 ...
 * }</pre>
 *
 * <p>File events are debounced to prevent duplicate processing when files are modified multiple
 * times in quick succession (e.g., during a text editor save). The debounce delay is configurable
 * via WatcherConfig.
 *
...
public class FileSystemDeploymentWatcher implements DeploymentWatcher {
  ...
  /**
   * Schedules a debounced event for a file. If a pending event exists for the file, it is cancelled
   * and replaced.
   *
