...
/**
 * Comprehensive unit tests for AutoDeploymentHandler. Tests auto-deploy, auto-start, error
 * handling, and listener notifications.
 */
class AutoDeploymentHandlerTest {
  ...
  void testOnDescriptorDetectedWithAutoDeployOnly() throws Exception {
    config =
        WatcherConfig.builder().watchDirectory(tempDir).autoDeploy(true).autoStart(false).build();
    handler = new AutoDeploymentHandler(mockApplicationManager, parsers, config);
  ...
  void testOnDescriptorDetectedWithAutoDeployDisabled() throws Exception {
    config =
        WatcherConfig.builder().watchDirectory(tempDir).autoDeploy(false).autoStart(true).build();
    handler = new AutoDeploymentHandler(mockApplicationManager, parsers, config);
  ...
  void testOnDescriptorDetectedDeployException() throws Exception {
    ...
    doThrow(new RuntimeException("Deploy failed"))
        .when(mockApplicationManager)
        .deploy(mockDescriptor);
