...
/**
 * Comprehensive unit tests for ApplicationApiHandler. Tests all application management endpoints,
...
class ApplicationApiHandlerTest {
  ...
  void testHandleDeployApplication() throws Exception {
    ...
    ApplicationContext mockContext =
        createMockApplicationContext("test-app", ApplicationState.DEPLOYED);
    when(mockManager.getApplicationContext("test-app")).thenReturn(mockContext);

    handler.handle(exchange);

    ArgumentCaptor<ApplicationDescriptor> descriptorCaptor =
        ArgumentCaptor.forClass(ApplicationDescriptor.class);
  ...
  void testHandleGetApplication() throws IOException {
    ...
    ApplicationContext mockContext =
        createMockApplicationContext("test-app", ApplicationState.RUNNING);
    when(mockManager.getApplicationContext("test-app")).thenReturn(mockContext);
  ...
  void testHandleGetStatusNotRunning() throws IOException {
    ...
    ApplicationContext mockContext =
        createMockApplicationContext("test-app", ApplicationState.STOPPED);
    when(mockManager.getApplicationContext("test-app")).thenReturn(mockContext);
  ...
  void testHandleStopApplicationException() throws Exception {
    ...
    doThrow(new RuntimeException("Stop failed")).when(mockManager).stop("test-app");
  ...
  void testHandleUndeployApplicationException() throws Exception {
    ...
    doThrow(new RuntimeException("Undeploy failed")).when(mockManager).undeploy("test-app");
  ...
  void testApplicationDescriptorDtoConversion() {
    ApplicationApiHandler.ApplicationDescriptorDTO dto =
        new ApplicationApiHandler.ApplicationDescriptorDTO();
  ...
  void testExtractAppIdFromPath() throws IOException {
    ...
    ApplicationContext mockContext =
        createMockApplicationContext("my-app-123", ApplicationState.DEPLOYED);
  ...
  private HttpExchange createMockExchange(String method, String path, String requestBody)
      throws IOException {
