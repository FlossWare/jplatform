 ...
 * in the configuration. Returns 401 Unauthorized if authentication fails, otherwise passes the
 * request through.
 *
...
public class ApiAuthFilter extends Filter {
  ...
  /**
   * Filters the HTTP request and checks authentication if enabled. Allows OPTIONS requests to pass
   * through for CORS preflight.
   *
  ...
  /**
   * Performs constant-time comparison of two strings to prevent timing attacks. Uses
   * MessageDigest.isEqual which compares all bytes regardless of match.
   *
