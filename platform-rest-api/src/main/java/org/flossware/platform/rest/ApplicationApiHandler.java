 ...
 * starting, stopping, and querying applications.
 *
 * <p>Supported endpoints:
 *
 * <ul>
 *   <li>POST /api/applications - Deploy a new application
 *   <li>GET /api/applications - List all applications
 *   <li>GET /api/applications/{id} - Get application details
 *   <li>GET /api/applications/{id}/status - Get application status and metrics
 *   <li>POST /api/applications/{id}/start - Start an application
 *   <li>POST /api/applications/{id}/stop - Stop an application
 *   <li>DELETE /api/applications/{id} - Undeploy an application
 *   <li>GET /api/applications/{id}/metrics - Get resource metrics history
 * </ul>
 *
 * <p>Example deploy request:
 *
 * <pre>{@code
...
public class ApplicationApiHandler implements HttpHandler {
  ...
  private static final Pattern APP_STATUS_PATTERN =
      Pattern.compile("/api/applications/([^/]+)/status$");
  private static final Pattern APP_START_PATTERN =
      Pattern.compile("/api/applications/([^/]+)/start$");
  private static final Pattern APP_STOP_PATTERN =
      Pattern.compile("/api/applications/([^/]+)/stop$");
  private static final Pattern APP_METRICS_PATTERN =
      Pattern.compile("/api/applications/([^/]+)/metrics$");
  ...
  /**
   * Handles HTTP requests for application endpoints. Routes requests based on path and HTTP method.
   *
  ...
  /**
   * Handles POST /api/applications - deploy a new application. Accepts JSON ApplicationDescriptor
   * in the request body.
   *
   * @param exchange the HTTP exchange
   * @throws IOException if an I/O error occurs
  ...
  private void handleDeploy(HttpExchange exchange) throws IOException {
    try (InputStream is = exchange.getRequestBody()) {
      ...
      ApplicationDescriptorDTO dto =
          mapper.readValue(limitedStream, ApplicationDescriptorDTO.class);
  ...
  /**
   * Handles GET /api/applications - list all applications. Returns a list of all deployed
   * applications with their current states.
   *
   * @param exchange the HTTP exchange
   * @throws IOException if an I/O error occurs
   */
  ...
  /**
   * Handles GET /api/applications/{id} - get application details. Returns full application
   * information including metrics if running.
   *
   * @param exchange the HTTP exchange
  ...
  /**
   * Handles GET /api/applications/{id}/status - get application status. Returns current state and
   * runtime metrics.
   *
   * @param exchange the HTTP exchange
   * @param appId the application identifier
  ...
  /**
   * Handles POST /api/applications/{id}/start - start an application. Transitions the application
   * from DEPLOYED or STOPPED to RUNNING state.
   *
   * @param exchange the HTTP exchange
   * @param appId the application identifier
   * @throws IOException if an I/O error occurs
  ...
  /**
   * Handles POST /api/applications/{id}/stop - stop an application. Transitions the application
   * from RUNNING to STOPPED state.
   *
   * @param exchange the HTTP exchange
   * @param appId the application identifier
   * @throws IOException if an I/O error occurs
  ...
  /**
   * Handles DELETE /api/applications/{id} - undeploy an application. Removes the application from
   * the platform and cleans up resources.
   *
   * @param exchange the HTTP exchange
   * @param appId the application identifier
   * @throws IOException if an I/O error occurs
  ...
  /**
   * Handles GET /api/applications/{id}/metrics - get resource metrics history. Returns historical
  ...
  private void handleGetMetrics(HttpExchange exchange, String appId) throws IOException {
    ...
    try {
      ...
      sendErrorResponse(
          exchange, 404, "NotAvailable", "Metrics history not available for this application");
      return;
    }
   ...
   * @throws IOException if an I/O error occurs
   */
  private void sendJsonResponse(HttpExchange exchange, int statusCode, Object data)
      throws IOException {
  ...
  private void sendErrorResponse(
      HttpExchange exchange, int statusCode, String error, String message) throws IOException {
    ErrorResponseDTO errorResponse = ErrorResponseDTO.create(error, message, statusCode);
    sendJsonResponse(exchange, statusCode, errorResponse);
  }

  /**
   * Data transfer object for application descriptor in deploy requests. Simplified version for JSON
   * deserialization.
   */
  public static class ApplicationDescriptorDTO {
    ...
    public ApplicationDescriptor toApplicationDescriptor() {
      ...
      ApplicationDescriptor.Builder builder =
          ApplicationDescriptor.builder().applicationId(applicationId).mainClass(mainClass);
  ...
  /**
   * InputStream wrapper that enforces a maximum read size. Throws RequestTooLargeException if the
   * limit is exceeded.
   */
