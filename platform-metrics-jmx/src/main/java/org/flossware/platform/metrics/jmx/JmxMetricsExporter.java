 ...
 * Registers MBeans for each application and optionally creates an RMI registry for remote
 * monitoring.
 *
 * <p>Features:
 *
 * <ul>
 *   <li>Automatic MBean registration for deployed applications
 *   <li>Optional RMI registry for remote JMX access
 *   <li>Thread-safe registration and unregistration
 *   <li>Customizable JMX domain for MBean ObjectNames
 * </ul>
 *
 * <p>ObjectName pattern:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Thread safety: This class is thread-safe. All registration operations are protected by
 * internal synchronization using ConcurrentHashMap.
 *
 * @see MetricsExporter
 * @see ApplicationMBean
 * @see org.flossware.platform.api.JmxExporterConfig
 */
public class JmxMetricsExporter implements MetricsExporter {
  ...
  /**
   * Constructs a new JMX metrics exporter with the specified configuration. Uses the platform MBean
   * server for MBean registration.
   *
  ...
  public void registerApplication(String applicationId, ApplicationContext context) {
    ...
    try {
      ...
      String objectNameStr =
          String.format("%s:type=Application,id=%s", config.getDomain(), quotedAppId);
