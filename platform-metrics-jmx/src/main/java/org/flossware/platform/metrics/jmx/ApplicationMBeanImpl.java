...
import org.flossware.platform.api.ResourceUsageHistory;
 ...
 * access to the ApplicationContext and operations to the ApplicationManager.
 *
 * <p>This implementation provides:
 *
 * <ul>
 *   <li>Real-time resource monitoring (CPU, memory, threads)
 *   <li>Thread pool statistics (active, queued, completed tasks)
 *   <li>Lifecycle operations (start, stop)
 *   <li>Historical resource data in JSON format
 * </ul>
 *
 * <p>Thread safety: This class is thread-safe as it delegates to thread-safe components
 * (ApplicationContext and ApplicationManager).
 *
 * <p>Example usage:
 *
 * <pre>{@code
 ...
 * @see org.flossware.platform.api.ApplicationContext
 * @see org.flossware.platform.api.PlatformManager
 */
public class ApplicationMBeanImpl implements ApplicationMBean {
  ...
  public ApplicationMBeanImpl(
      String applicationId, ApplicationContext context, PlatformManager manager) {
  ...
  public String getResourceHistory(int minutes) {
    ...
    try {
      ...
      List<Map<String, Object>> snapshots =
          history.getSnapshots().stream().map(this::snapshotToMap).collect(Collectors.toList());
