# platform-java VM Management

Virtual machine management module for platform-java using libvirt/KVM/QEMU.

## Overview

This module enables platform-java to manage VMs alongside containers, Java applications, and native binaries, providing **unified orchestration across all workload types**.
...
<dependency>
    ...
    <artifactId>platform-java-vm-management</artifactId>
...
properties:
  ...
  vm.disk: /var/lib/platform-java/vms/db.qcow2
  vm.network: bridge
...
```bash
# Deploy via platform-java
platform-java deploy database-vm.yaml

# Start VM
platform-java start database-vm

# View status
platform-java status database-vm

# Stop VM (graceful)
platform-java stop database-vm

# Force stop
platform-java stop database-vm --force
```
...
import org.flossware.platform-java.api.ApplicationManager;
...
ApplicationDescriptor vmDescriptor = ApplicationDescriptor.builder()
    ...
    .property("vm.disk", "/var/lib/platform-java/vms/web.qcow2")
...
| `vm.disk` | Path to disk image | **required** | `/var/lib/platform-java/vms/vm.qcow2` |
...
VMs can depend on other VMs, containers, or any platform-java workload:
...
platform-java monitors and enforces resource limits for VMs:
...
VMs export metrics to Prometheus like other platform-java workloads:

```
# VM metrics
platform-java_vm_cpu_time_seconds{vm="database-vm"}
platform-java_vm_memory_mb{vm="database-vm"}
platform-java_vm_state{vm="database-vm", state="running"}
```

**View metrics:**
```bash
# Via CLI
platform-java metrics database-vm
...
# Copy as VM disk
cp ubuntu-22.04-server-cloudimg-amd64.img /var/lib/platform-java/vms/myvm.qcow2

# Resize if needed
qemu-img resize /var/lib/platform-java/vms/myvm.qcow2 50G
```

**Option 2: Create new empty disk**
```bash
# Create 50GB qcow2 disk
qemu-img create -f qcow2 /var/lib/platform-java/vms/myvm.qcow2 50G
```

**Option 3: Convert from other format**
```bash
# Convert VMDK to qcow2
qemu-img convert -f vmdk -O qcow2 source.vmdk /var/lib/platform-java/vms/myvm.qcow2
```

## Integration with VirtOS

When platform-java runs on VirtOS, VM management is pre-configured:

```bash
# VirtOS includes platform-java with VM support
virtos-tui
  → Workloads (platform-java)
     → Virtual Machines
        → Deploy VM
        → List VMs
        → VM Monitoring

# All VMs managed via platform-java
# VirtOS provides the infrastructure (storage, networking, multi-cloud)
# platform-java provides the orchestration
```
...
## Comparison: platform-java VM vs. Direct libvirt

| Feature | Direct libvirt | platform-java VM |
|---------|----------------|--------------|
| **Create VM** | `virsh define`, `virsh start` | `platform-java deploy vm.yaml` |
...
```java
import org.flossware.platform-java.vm.VmLauncher;
...
```bash
# Live migration
platform-java migrate app-vm --destination qemu+ssh://host2/system

# Snapshots
platform-java snapshot create app-vm --name before-upgrade
platform-java snapshot list app-vm
platform-java snapshot revert app-vm --name before-upgrade
platform-java snapshot delete app-vm --name before-upgrade

# Hot-add resources
platform-java resize app-vm --vcpu 8 --memory 16384
```
...
See main platform-java CONTRIBUTING.md for contribution guidelines.
