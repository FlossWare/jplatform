 ...
 *
 * <ul>
 *   <li>Registers services with Eureka using REST API
 *   <li>Sends periodic heartbeats to maintain registrations
 *   <li>Maintains local cache for fast service lookups
 *   <li>Publishes service availability while keeping local instances
 * </ul>
 *
 * <p>Thread Safety: This class is thread-safe. All mutable state is protected by concurrent
 * collections and volatile flags.
 *
 * @since 1.1
 */
public class EurekaServiceRegistry implements ServiceRegistry, AutoCloseable {
  ...
  /** Starts the Eureka service registry. Must be called before using the registry. */
  public void start() {
    if (started) {
      return;
    }
  ...
  private void registerWithEureka(Class<?> serviceInterface) {
    try {
      ...
      instance.put(
          "metadata", Collections.singletonMap("service-class", serviceInterface.getName()));
