 ...
 * service catalog, sessions, and key/value store for leader election and membership.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Service registration with Consul agent
 *   <li>Leader election via Consul sessions and KV locks
 *   <li>Automatic health checking
 *   <li>Event notifications for membership changes
 *   <li>Session renewal to maintain cluster membership
 * </ul>
 *
 * <p>Example usage:
 *
...
public class ConsulClusterManager implements ClusterManager {
  ...
  /**
   * Joins the cluster with the specified configuration. Registers this node as a service in Consul,
   * creates a session, and attempts leader election.
   *
            ...
            Consul.builder()
                .withHostAndPort(
                    com.google.common.net.HostAndPort.fromParts(
                        consulConfig.getConsulHost(), consulConfig.getConsulPort()))
                .build();
      }
  ...
  /**
   * Leaves the cluster gracefully. Deregisters the service, destroys the session, and releases
   * resources.
   *
  ...
  /**
   * Returns all nodes currently in the cluster. Queries Consul's health API for all healthy
  ...
  public Set<ClusterNode> getNodes() {
    ...
    try {
      ...
      List<ServiceHealth> services =
          healthClient.getHealthyServiceInstances(consulConfig.getServiceName()).getResponse();
  ...
  /**
   * Checks if this node is the cluster leader. Leadership is determined by holding the leader key
   * in Consul KV with this node's session.
   *
  ...
  /**
   * Closes the cluster manager and releases all resources. Equivalent to calling leave().
   *
  ...
  /**
   * Registers this node as a service in Consul. Creates a unique service ID and registers with
   * health check.
   */
  ...
  /**
   * Creates a Consul session for this node. The session is used for leader election and will
  ...
  /**
   * Renews the Consul session to prevent expiration. This method is called periodically by the
   * scheduler.
   */
  ...
  /**
   * Watches for membership changes in the cluster. This method is called periodically by the
   * scheduler.
   */
