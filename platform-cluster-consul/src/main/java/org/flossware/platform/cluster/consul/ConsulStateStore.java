 ...
 * state and descriptors using Consul's key/value store.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Distributed storage with Consul KV
 *   <li>JSON serialization for ApplicationDescriptor
 *   <li>Thread-safe concurrent access
 *   <li>Watch-based state change notifications
 * </ul>
 *
 * <p>Uses Consul KV paths:
 *
 * <ul>
 *   <li>"jplatform/state/{appId}" - stores ApplicationState enum name
 *   <li>"jplatform/descriptor/{appId}" - stores JSON-serialized ApplicationDescriptor
 * </ul>
 *
 * <p>Example usage:
 *
...
public class ConsulStateStore implements ClusterStateStore {
  ...
  /**
   * Stores application state in Consul KV. The state enum name is stored as a string value.
   *
  ...
  /**
   * Returns all application states in the cluster. Queries all keys under the state prefix.
   *
  ...
  /**
   * Stores an application descriptor in Consul KV. The descriptor is serialized to JSON before
   * storage.
   *
   * @param applicationId the application identifier
  ...
  /**
   * Retrieves an application descriptor from Consul KV. The descriptor is deserialized from JSON.
   *
   * @param applicationId the application identifier
  ...
  /**
   * Returns all application descriptors in the cluster. Queries all keys under the descriptor
   * prefix and deserializes them.
   *
  ...
  /**
   * Subscribes to state changes for a specific application. The listener will be notified whenever
   * the state changes detected via Consul watch.
   *
   * <p>Implementation uses Consul polling to detect changes from other cluster nodes. A background
   * watch task polls Consul every 5 seconds for state changes.
   *
  ...
  /**
   * Unsubscribes from state changes for a specific application. Stops the watch task if this is the
   * last listener.
   *
  ...
  /**
   * Watches a key for changes and notifies listeners. Uses simple polling - checks the value every
   * execution.
   *
   * @param applicationId the application ID to watch
   */
