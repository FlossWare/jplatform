 ...
 * topic-based publish-subscribe messaging using a JMS broker, enabling applications across multiple
 * platform-java nodes to communicate.
 *
 * <p>Features:
 *
 * <ul>
 *   <li>Multi-node messaging via JMS broker
 *   <li>Persistent messages (survive broker restarts)
 *   <li>Guaranteed delivery (broker handles durability)
 *   <li>Compatible with any JMS 3.0+ broker (ActiveMQ Artemis, etc.)
 * </ul>
 *
 * <p>Example usage:
 *
 ...
 * }</pre>
 *
 * <p><b>Thread Safety:</b> This class is thread-safe. Uses ConcurrentHashMap for subscriptions map
 * and CopyOnWriteArrayList for subscription lists. All JMS operations are thread-safe.
 *
 * <p><b>Message Format:</b> Platform messages are serialized as JMS BytesMessage with headers:
 *
 * <ul>
 *   <li>JMSMessageID: Platform message ID
 *   <li>JMSTimestamp: Platform message timestamp
 *   <li>sourceApplicationId: Custom property for source app
 *   <li>Custom headers: Stored as JMS properties (String keys, Object values)
 *   <li>Payload: Binary data in message body
 * </ul>
 *
 * @see MessageBus
 * @see JmsConfig
...
public class JmsMessageBus implements MessageBus, AutoCloseable {
  ...
  public JmsMessageBus(JmsConfig config, ConnectionFactory connectionFactory) throws JMSException {
    ...
    if (config.getUsername() != null && config.getPassword() != null) {
      this.connection =
          connectionFactory.createConnection(config.getUsername(), config.getPassword());
    } else {
    ...
    this.publishSession =
        connection.createSession(config.isTransacted(), config.getAcknowledgeMode());
  ...
  /**
   * Publishes a message to the specified topic via JMS. The message is sent to all subscribers
   * across all nodes connected to the JMS broker.
   *
  ...
  /**
   * Subscribes to receive messages on the specified topic. Creates a JMS MessageConsumer that
   * receives messages from all publishers across all nodes connected to the broker.
   *
  ...
  public Subscription subscribe(String topic, MessageHandler handler) {
    ...
    try {
      ...
      JmsSubscriptionImpl subscription =
          new JmsSubscriptionImpl(topic, handler, session, consumer, this);
  ...
  /**
   * Unsubscribes the specified subscription. Closes the JMS MessageConsumer and session associated
  ...
  /**
   * Internal method to remove a subscription from the subscriptions map. Called by
   * JmsSubscriptionImpl when cancelled.
   *
  ...
  /**
   * Closes the JMS message bus and releases all resources. Cancels all subscriptions, closes all
   * sessions, and closes the connection.
   *
   ...
   */
  private Message fromJmsMessage(jakarta.jms.Message jmsMessage, String topic)
