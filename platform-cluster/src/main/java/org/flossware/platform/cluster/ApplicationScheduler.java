 ...
 * leader and is responsible for distributing applications across available nodes.
 *
 * <p>Key responsibilities:
 *
 * <ul>
 *   <li>Assign new applications to nodes using load balancing
 *   <li>Track application assignments in distributed storage
 *   <li>Handle node failures by rescheduling affected applications
 *   <li>Provide node selection algorithms (round-robin, least-loaded)
 * </ul>
 *
 * <p>Uses Hazelcast IMap for storing assignments:
 *
 * <ul>
 *   <li>"jplatform-application-assignments" - maps application ID to node ID
 *   <li>"jplatform-node-load" - tracks application count per node
 * </ul>
 *
 * <p>Example usage:
 *
...
public class ApplicationScheduler {
  ...
  public ApplicationScheduler(
      HazelcastInstance hazelcast, ClusterManager clusterManager, SchedulingStrategy strategy) {
  ...
  /**
   * Assigns an application to a cluster node. Only the cluster leader should call this method. Uses
   * the configured scheduling strategy to select the target node.
   *
  ...
  /**
   * Unassigns an application from its current node. Only the cluster leader should call this
   * method.
   *
   * @param applicationId the application identifier
   * @throws IllegalStateException if not called on the leader node
  ...
  /**
   * Reassigns applications from a failed node to other available nodes. Only the cluster leader
  ...
  public int reassignFromFailedNode(String failedNodeId) {
    ...
    for (String appId : affectedApps) {
      try {
        ...
        nodeLoadMap.compute(
            failedNodeId, (k, count) -> count == null || count <= 1 ? null : count - 1);
