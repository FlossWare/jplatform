 ...
 * and excludes non-serializable fields like SecurityConfig.
 */
public class ApplicationDescriptorJsonModule extends SimpleModule {
  ...
  /**
   * Custom serializer for ApplicationDescriptor. Excludes SecurityConfig to avoid FilePermission
   * serialization issues.
   */
  private static class ApplicationDescriptorSerializer
      extends JsonSerializer<ApplicationDescriptor> {
    @Override
    public void serialize(
        ApplicationDescriptor descriptor, JsonGenerator gen, SerializerProvider serializers)
        throws IOException {
  ...
  /** Custom deserializer for ApplicationDescriptor. Reconstructs using the Builder pattern. */
  private static class ApplicationDescriptorDeserializer
      extends JsonDeserializer<ApplicationDescriptor> {
    @Override
    public ApplicationDescriptor deserialize(JsonParser parser, DeserializationContext ctxt)
      ...
      if (props != null && props.isObject()) {
        props
            .fields()
            .forEachRemaining(entry -> builder.property(entry.getKey(), entry.getValue().asText()));
      }
