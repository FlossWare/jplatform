 ...
 * state and descriptors using Hazelcast's distributed maps (IMap).
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Distributed storage with automatic replication
 *   <li>Event notifications on state changes
 *   <li>JSON serialization for ApplicationDescriptor
 *   <li>Thread-safe concurrent access
 * </ul>
 *
 * <p>Uses two Hazelcast IMaps:
 *
 * <ul>
 *   <li>"jplatform-application-states" - stores ApplicationState by ID
 *   <li>"jplatform-application-descriptors" - stores serialized ApplicationDescriptor by ID
 * </ul>
 *
 * <p>Example usage:
 *
...
public class HazelcastStateStore implements ClusterStateStore {
  ...
  /**
   * Stores application state in the distributed map. The state is replicated across all cluster
   * nodes.
   *
  ...
  /**
   * Returns all application states in the cluster. Creates a local snapshot of the distributed map.
   *
  ...
  /**
   * Stores an application descriptor in the distributed map. The descriptor is serialized to JSON
   * before storage.
   *
   * @param applicationId the application identifier
  ...
  /**
   * Retrieves an application descriptor from the distributed map. The descriptor is deserialized
   * from JSON.
   *
   * @param applicationId the application identifier
  ...
  /**
   * Returns all application descriptors in the cluster. Creates a local snapshot and deserializes
   * all descriptors.
   *
      ...
      try {
        ...
        throw new RuntimeException(
            "Failed to deserialize application descriptor for "
                + entry.getKey()
                + " - cluster state may be corrupted",
            e);
      }
    }
   ...
   * the state changes in the distributed map.
   *
  ...
  public void subscribe(String key, StateChangeListener listener) {
    ...
    UUID listenerId = stateMap.addEntryListener(new StateMapListener(listener), key, true);
  ...
  private class StateMapListener
      implements EntryAddedListener<String, ApplicationState>,
