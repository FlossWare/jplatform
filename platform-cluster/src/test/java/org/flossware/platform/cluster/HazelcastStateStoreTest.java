 ...
 * serialization, and state change listeners.
 */
class HazelcastStateStoreTest {
  ...
  void setUp() {
    ...
    when(mockHazelcast.<String, ApplicationState>getMap("jplatform-application-states"))
        .thenReturn(mockStateMap);
    when(mockHazelcast.<String, String>getMap("jplatform-application-descriptors"))
        .thenReturn(mockDescriptorMap);
  ...
  void testGetApplicationDescriptor() {
    ...
    String appId = "test-app";
    String json =
        "{\"applicationId\":\"test-app\",\"name\":\"Test App\",\"version\":\"1.0.0\","
  ...
  void testGetAllApplicationDescriptors() {
    ...
    jsonMap.put(
        "app1",
        "{\"applicationId\":\"app1\",\"name\":\"App 1\",\"version\":\"1.0.0\","
            + "\"mainClass\":\"com.test.Main1\",\"classpath\":[],\"environmentVariables\":{},\"systemProperties\":{}}");
    jsonMap.put(
        "app2",
        "{\"applicationId\":\"app2\",\"name\":\"App 2\",\"version\":\"2.0.0\","
  ...
  void testGetAllApplicationDescriptorsWithInvalid() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> stateStore.getAllApplicationDescriptors());
  ...
  void testSubscribe() {
    ...
    when(mockStateMap.addEntryListener(any(), eq(appId), eq(true))).thenReturn(listenerId);
  ...
  void testConcurrentSubscriptions() throws InterruptedException {
    ...
    when(mockStateMap.addEntryListener(any(), eq(appId), eq(true)))
        .thenReturn(
            listenerIds.get(0), listenerIds.subList(1, listenerIds.size()).toArray(new UUID[0]));
