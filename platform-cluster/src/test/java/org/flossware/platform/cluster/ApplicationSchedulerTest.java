 ...
 * strategies, node failure handling, and load balancing.
 */
class ApplicationSchedulerTest {
  ...
  void setUp() {
    ...
    when(mockHazelcast.<String, String>getMap("jplatform-application-assignments"))
        .thenReturn(mockAssignmentMap);
    ...
    when(mockNodeLoadMap.getOrDefault(anyString(), anyInt()))
        .thenAnswer(
            invocation -> {
  ...
  void testRoundRobinStrategy() {
    ...
    scheduler =
        new ApplicationScheduler(
            mockHazelcast, mockClusterManager, ApplicationScheduler.SchedulingStrategy.ROUND_ROBIN);
  ...
  void testAssignUpdatesNodeLoad() {
    ...
    when(mockNodeLoadMap.compute(eq(nodeId), any()))
        .thenAnswer(
            invocation -> {
              java.util.function.BiFunction<String, Integer, Integer> func =
  ...
  void testReassignFromFailedNode() {
    ...
    when(mockNodeLoadMap.compute(anyString(), any()))
        .thenAnswer(
            invocation -> {
  ...
  void testReassignAsNonLeader() {
    ...
    // When/Then
    assertThrows(
        IllegalStateException.class, () -> scheduler.reassignFromFailedNode("failed-node"));
  }
  ...
  void testLoadBalancing() {
    ...
    when(mockNodeLoadMap.getOrDefault(anyString(), eq(0)))
        .thenAnswer(
            invocation -> {
