...
/**
 * Comprehensive unit tests for ClusteredApplicationManager. Tests cluster-aware
 * deploy/start/stop/undeploy operations and failure handling.
 */
class ClusteredApplicationManagerTest {
  ...
  void testCreateClusteredManager() {
    ...
    when(hzClusterManager.getHazelcastInstance())
        .thenReturn(mock(com.hazelcast.core.HazelcastInstance.class));

    // When
    manager =
        new ClusteredApplicationManager(
            mockMessageBus, mockServiceRegistry, hzClusterManager, mockStateStore);
  ...
  void testNodeJoinedEvent() throws Exception {
    ...
    ClusterNode newNode =
        new ClusterNode(
            "node-2", "host-2", 5702, ClusterNode.NodeState.ACTIVE, System.currentTimeMillis());
  ...
  void testNodeLeftEventAsLeader() throws Exception {
    ...
    ClusterNode leftNode =
        new ClusterNode(
            "failed-node", "host-2", 5702, ClusterNode.NodeState.DEAD, System.currentTimeMillis());
  ...
  void testGetClusterManager() {
    ...
    manager =
        new ClusteredApplicationManager(
            mockMessageBus, mockServiceRegistry, mockClusterManager, mockStateStore);
  ...
  void testDeploymentErrorClustered() throws Exception {
    ...
    doThrow(new RuntimeException("State store error"))
        .when(mockStateStore)
        .putApplicationDescriptor(anyString(), any());
  ...
  private void setupClusteredManager() {
    HazelcastClusterManager hzClusterManager = mock(HazelcastClusterManager.class);
    com.hazelcast.core.HazelcastInstance mockHazelcast =
        mock(com.hazelcast.core.HazelcastInstance.class);
  ...
  private ApplicationDescriptor createTestDescriptor(String appId) {
    return ApplicationDescriptor.builder()
        ...
        .name(appId + "-name")
        .version("1.0.0")
        .mainClass("org.flossware.platform.cluster.TestApp")
