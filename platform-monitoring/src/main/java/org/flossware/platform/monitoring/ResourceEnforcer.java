 ...
 *
 * <p>This component implements the enforcement side of resource management. While {@link
 * ApplicationResourceMonitor} handles monitoring and detection, ResourceEnforcer handles the
 * action-taking when quotas are exceeded.
 *
 * <p>Enforcement actions range from passive notification to active throttling or shutdown of the
 * violating application. Each resource type (CPU, memory, threads) can have its own enforcement
 * policy.
 *
 * <p>Grace periods prevent transient resource spikes from triggering enforcement. Actions are only
 * taken after consecutive violations over multiple monitoring cycles.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public class ResourceEnforcer {
   ...
   * @throws NullPointerException if applicationId, config, threadGroup, shutdownAction, or
   *     killAction is null
   */
  public ResourceEnforcer(
      String applicationId,
   ...
   *
   * <p>This method should be called by the monitoring thread when a quota violation is detected. It
   * implements grace period logic and triggers the appropriate enforcement action based on
   * configuration.
   *
  ...
  /**
   * Enforces heap memory quota. Checks if current heap usage exceeds the configured limit and takes
  ...
  /**
   * Enforces CPU time quota. Checks if cumulative CPU time exceeds the configured limit and takes
   * the configured enforcement action after grace period.
   *
   * @param quota the resource quota
   * @param snapshot the current snapshot
  ...
  /**
   * Enforces thread count quota. Checks if current thread count exceeds the configured limit and
   * takes the configured enforcement action after grace period.
   *
   * @param quota the resource quota
   * @param snapshot the current snapshot
   */
  ...
  /**
   * Returns the enforcement policy for this application. Useful for inspecting violation counts and
   * grace period state.
   *
