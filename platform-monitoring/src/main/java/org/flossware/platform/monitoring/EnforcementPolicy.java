 ...
 *
 * <p>This class implements the grace period logic - enforcement actions are only triggered after a
 * resource has been in violation for a configured number of consecutive monitoring cycles. This
 * prevents transient spikes from causing unnecessary disruptions.
 *
 * <p>Each application has its own enforcement policy instance that tracks violation counts
 * independently for each resource type (CPU, memory, threads).
 *
 * <p>Thread-safe for concurrent access from monitoring threads.
 *
 * @since 2.0
 */
public class EnforcementPolicy {
  ...
  /**
   * Records a quota violation for a resource type. Returns true if the grace period has been
   * exceeded and enforcement should be triggered.
   *
  ...
  /**
   * Clears the violation count for a resource type. Called when a resource is back within quota
   * limits.
   *
  ...
  /** Clears all violation counts. Used when enforcement is triggered to reset the grace period. */
