 ...
 * and JavaScript files from the classpath resources under /web/. It provides routing for the web
 * console interface:
 *
 * <ul>
 *   <li>/ or /console → index.html
 *   <li>/static/* → CSS/JS files from /web/static/
 *   <li>Other paths → served from /web/ directory
 * </ul>
 *
 * Example usage:
 *
...
public class WebConsoleHandler implements HttpHandler {
  ...
  /**
   * Handles incoming HTTP requests and serves static files from the classpath. The handler maps URL
   * paths to classpath resources:
   *
   * <ul>
   *   <li>Root path (/) and /console → /web/index.html
   *   <li>All other paths → /web/{path}
   * </ul>
   *
