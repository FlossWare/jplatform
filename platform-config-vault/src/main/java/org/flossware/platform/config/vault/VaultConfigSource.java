 ...
 *
 * <ul>
 *   <li>Stores configuration in Vault secrets engine
 *   <li>Supports token-based authentication
...
public class VaultConfigSource implements AutoCloseable {
  ...
  /**
   * Starts the Vault configuration source. Must be called before using the configuration source.
   */
  public void start() {
    if (started) {
      return;
    ...
    try {
      ...
      vault =
          new Vault(vaultConfig).withRetries(config.getMaxRetries(), config.getRetryIntervalMs());
  ...
  /** Refreshes the configuration from Vault. Reloads all secrets from the Vault server. */
  public void refresh() {
    if (!started) {
      throw new IllegalStateException("Config source not started");
    }
