...
import org.flossware.platform.fswatcher.AutoDeploymentHandler;
import org.flossware.platform.fswatcher.FileSystemDeploymentWatcher;
import org.flossware.platform.messaging.InMemoryMessageBus;
import org.flossware.platform.messaging.ServiceRegistryImpl;
import org.flossware.platform.metrics.jmx.JmxMetricsExporter;
import org.flossware.platform.metrics.prometheus.PrometheusMetricsExporter;
import org.flossware.platform.rest.JdkHttpApiServer;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Main entry point for JPlatform. Bootstraps the platform and provides interactive management
 * console with optional features.
 *
 * <p>This launcher initializes the platform infrastructure (message bus, service registry, and
 * application manager) and provides an interactive command-line interface for managing
 * applications. Applications can be deployed, started, stopped, and monitored through the console.
 *
 * <p>Optional features can be enabled via command-line arguments:
 *
 * <ul>
 *   <li>--rest-api - Enable REST API server (default port 8080)
 *   <li>--port &lt;number&gt; - Specify port for REST API
 *   <li>--web-console - Enable web console (requires --rest-api)
 *   <li>--jmx-port &lt;number&gt; - Enable JMX metrics on specified port
 *   <li>--prometheus - Enable Prometheus metrics (default port 9090)
 *   <li>--prometheus-port &lt;number&gt; - Specify port for Prometheus metrics
 *   <li>--watch-dir &lt;path&gt; - Enable filesystem watcher for auto-deployment
 * </ul>
 *
 * <p>Supported commands:
 *
 * <ul>
 *   <li>deploy - Deploy an application from a JAR file
 *   <li>deploy-yaml - Deploy an application from a YAML descriptor
 *   <li>deploy-json - Deploy an application from a JSON descriptor
 *   <li>start - Start a deployed application
 *   <li>stop - Stop a running application
 *   <li>undeploy - Undeploy an application
 *   <li>list - List all applications and their states
 *   <li>status - Show detailed status of an application
 *   <li>exit - Exit the platform
...
public class PlatformLauncher {
  ...
  private org.flossware.platform.otel.OpenTelemetryMetricsExporter otelExporter;
  ...
  private static class LauncherConfig {
    ...
    private static void printStartupHelp() {
      ...
      System.out.println(
          "  --config <file>         Load configuration from YAML file (default: platform.yaml)");
      System.out.println("  --rest-api              Enable REST API server (default port 8080)");
      System.out.println(
          "  --port <number>         Specify port for REST API (implies --rest-api)");
      ...
      System.out.println(
          "  java -jar jplatform-launcher.jar --rest-api --port 9090 --jmx-port 9999");
      System.out.println(
          "  java -jar jplatform-launcher.jar --prometheus --watch-dir /var/platform/apps");
    }
  }
  ...
  private void initializeOptionalComponents() {
    ...
    if (config.watcherEnabled) {
      try {
        ...
        AutoDeploymentHandler handler =
            new AutoDeploymentHandler(applicationManager, parsers, watcherConfig);
   ...
   *
   * <p>Note: Lifecycle listener support not yet implemented (see issue #340). For now, applications
   * are registered manually in deploy commands.
   */
   ...
   *
   * <p>This method runs in a loop, reading commands from standard input and dispatching them to the
   * appropriate handlers. The loop continues until the 'exit' or 'quit' command is received.
   */
  public void start() {
    printWelcome();
  ...
  private void handleDeployYaml(String args) throws Exception {
    ...
    if (jmxExporter != null) {
      ApplicationContext context =
          applicationManager.getApplicationContext(descriptor.getApplicationId());
   ...
   *
   * <p>Creates and starts the platform launcher. If a fatal error occurs during initialization or
   * execution, logs the error and exits with code 1.
   *
