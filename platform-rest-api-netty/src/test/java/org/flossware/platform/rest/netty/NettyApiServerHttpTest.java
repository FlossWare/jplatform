...
/**
 * Integration tests for NettyApiServer HTTP request handling. Tests actual HTTP requests and
 * responses via HttpRequestHandler.
 */
class NettyApiServerHttpTest {
  ...
  void testHandlerThrowsGenericException() throws Exception {
    ...
    assertFalse(
        response.body().contains("Simulated server error"),
        "Error details should not leak to client");
    assertTrue(
        response.headers().firstValue("Content-Type").orElse("").contains("application/json"));
  }
  ...
  void testGetRequest() throws Exception {
    ...
    HttpRequest request =
        HttpRequest.newBuilder().uri(URI.create(BASE_URL + "/api/info")).GET().build();
  ...
  void testJsonResponse() throws Exception {
    ...
    server = new NettyApiServer(config);
    server.addRoute("/api/data", input -> "{\"name\":\"test\",\"value\":123,\"active\":true}");
    server.start();
  ...
  void testConcurrentRequests() throws Exception {
    NettyApiServerConfig config =
        NettyApiServerConfig.builder().port(TEST_PORT).workerThreads(4).build();

    server = new NettyApiServer(config);
    server.addRoute(
        "/api/concurrent", input -> "{\"thread\":\"" + Thread.currentThread().getName() + "\"}");
    server.start();
