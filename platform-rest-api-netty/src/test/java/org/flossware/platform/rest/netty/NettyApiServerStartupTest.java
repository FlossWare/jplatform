...
/**
 * Unit tests for NettyApiServer startup, validation, and lifecycle. Tests server start/stop
 * behavior and route modification safety.
 */
class NettyApiServerStartupTest {
  ...
  void testStartAndStop() throws Exception {
    NettyApiServerConfig config = NettyApiServerConfig.builder().port(TEST_PORT).build();
  ...
  void testAddRouteWhileRunning() throws Exception {
    ...
    try {
      ...
      IllegalStateException exception =
          assertThrows(IllegalStateException.class, () -> server.addRoute("/test", handler));
  ...
  void testRemoveRouteWhileRunning() throws Exception {
    ...
    try {
      IllegalStateException exception =
          assertThrows(IllegalStateException.class, () -> server.removeRoute("/test"));
  ...
  void testStartWithCustomThreadCounts() throws Exception {
    NettyApiServerConfig config =
        NettyApiServerConfig.builder().port(TEST_PORT).bossThreads(2).workerThreads(4).build();
  ...
  void testStartWithZeroWorkerThreads() throws Exception {
    ...
    NettyApiServerConfig config =
        NettyApiServerConfig.builder().port(TEST_PORT).workerThreads(0).build();
  ...
  void testStartWithIpv4Address() throws Exception {
    NettyApiServerConfig config =
        NettyApiServerConfig.builder().host("127.0.0.1").port(TEST_PORT_2).build();
  ...
  void testStartWithBindAllAddress() throws Exception {
    NettyApiServerConfig config =
        NettyApiServerConfig.builder().host("0.0.0.0").port(TEST_PORT_3).build();
  ...
  void testAddRouteAfterStop() throws Exception {
    ...
    assertDoesNotThrow(() -> server.addRoute("/test", input -> "response"));
