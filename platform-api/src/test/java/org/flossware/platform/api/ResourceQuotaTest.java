...
/** Unit tests for ResourceQuota. Tests builder validation and quota enforcement. */
class ResourceQuotaTest {
  ...
  void testEnforceHeapQuotaExceeded() {
    ...
    ResourceQuotaExceededException exception =
        assertThrows(
  ...
  void testEnforceThreadCountQuotaExceeded() {
    ResourceQuota quota = ResourceQuota.builder().maxThreadCount(20).build();
