...
class ClusterNodeTest {
  ...
  void testConstructor_emptyAddress() {
    IllegalArgumentException exception =
        assertThrows(
            ...
            () ->
                new ClusterNode(
                    "node-1", "", 8080, ClusterNode.NodeState.ACTIVE, System.currentTimeMillis()));
  ...
  void testConstructor_nullState() {
    NullPointerException exception =
        assertThrows(
            ...
            () ->
                new ClusterNode("node-1", "192.168.1.100", 8080, null, System.currentTimeMillis()));
  ...
  void testConstructor_negativeLastHeartbeat() {
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () ->
                new ClusterNode("node-1", "192.168.1.100", 8080, ClusterNode.NodeState.ACTIVE, -1));
  ...
  void testConstructor_zeroLastHeartbeat() {
    ...
    ClusterNode node =
        new ClusterNode("node-1", "192.168.1.100", 8080, ClusterNode.NodeState.ACTIVE, 0);
  ...
  void testAllNodeStates() {
    ...
    for (ClusterNode.NodeState state : ClusterNode.NodeState.values()) {
      ClusterNode node = new ClusterNode("node-1", "192.168.1.100", 8080, state, timestamp);
  ...
  void testValidPortBoundaries() {
    ...
    ClusterNode node1 =
        new ClusterNode(
            "node-1", "192.168.1.100", 1, ClusterNode.NodeState.ACTIVE, System.currentTimeMillis());
