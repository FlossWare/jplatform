...
/**
 * Immutable snapshot of an application's resource usage at a point in time. Captures CPU time,
 * memory usage, thread count, I/O metrics, and custom metrics.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public class ResourceSnapshot {
   ...
   * @throws IllegalArgumentException if any metric value is negative or customMetrics contains null
   *     keys/values
   */
  public ResourceSnapshot(
    ...
    if (heapUsedBytes < -1) {
      throw new IllegalArgumentException(
          "heapUsedBytes cannot be negative (except -1 for N/A): " + heapUsedBytes);
    }
    if (threadCount < 0) {
  ...
  public String toString() {
    return String.format(
        "ResourceSnapshot{timestamp=%d, cpu=%dns, heap=%dB, threads=%d, read=%dB, written=%dB}",
