...
/**
 * Snapshot of thread pool statistics at a point in time. Captures active threads, completed tasks,
 * queue depth, and pool sizing.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public class ThreadPoolStats {
   ...
   * @throws IllegalArgumentException if any parameter is negative, if maximumPoolSize <
   *     corePoolSize, if poolSize > maximumPoolSize, or if activeThreads > poolSize
   */
  public ThreadPoolStats(
    ...
    if (poolSize > maximumPoolSize) {
      throw new IllegalArgumentException(
          "poolSize (" + poolSize + ") cannot exceed maximumPoolSize (" + maximumPoolSize + ")");
    }
    if (activeThreads > poolSize) {
      throw new IllegalArgumentException(
          "activeThreads (" + activeThreads + ") cannot exceed poolSize (" + poolSize + ")");
    }
  ...
  public String toString() {
    return String.format(
        "ThreadPoolStats{active=%d, completed=%d, queued=%d, " + "poolSize=%d, core=%d, max=%d}",
