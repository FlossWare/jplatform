...
/**
 * Configuration for Prometheus metrics exporter. Specifies HTTP port and path for exposing metrics
  ...
  public static class Builder {
    ...
    public Builder path(String path) {
      if (path == null || path.trim().isEmpty()) {
        throw new IllegalArgumentException("Metrics path cannot be null or empty");
      }
      if (!path.startsWith("/")) {
        throw new IllegalArgumentException("Metrics path must start with '/', got: " + path);
      }
      this.path = path;
      return this;
    }
