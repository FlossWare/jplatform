 ...
 * }</pre>
 *
 * <p><strong>CORS Security:</strong> The default CORS policy denies all origins. You must
 * explicitly whitelist origins using {@link Builder#addAllowedOrigin(String)}. Never use wildcard
 * {@code "*"} in production as it bypasses same-origin policy and creates XSS/CSRF vulnerabilities.
 *
...
public final class ApiServerConfig {
  ...
  private ApiServerConfig(Builder builder) {
    ...
    this.allowedOrigins =
        builder.allowedOrigins != null
            ? Set.copyOf(builder.allowedOrigins)
  ...
  public static class Builder {
     ...
     *
     * <p><strong>Security Warning:</strong> Avoid using wildcard {@code "*"} as it allows any
     * website to make authenticated requests to your API, creating XSS and CSRF attack vectors.
     * Always specify exact origins including protocol (e.g., {@code
    ...
    public Builder threadPoolSize(int size) {
      if (size < 1) {
        throw new IllegalArgumentException("Thread pool size must be at least 1, got: " + size);
      }
    ...
    public Builder maxThreadPoolSize(int size) {
      if (size < 1) {
        throw new IllegalArgumentException("Max thread pool size must be at least 1, got: " + size);
      }
