...
/**
 * Provides precise heap memory profiling for applications. Uses JVMTI to iterate over heap objects
 * and attribute memory usage to specific ClassLoaders.
 *
 * <p>This is an optional feature that requires a native JVMTI agent. If the agent is not available,
 * the platform falls back to estimation based on Runtime.totalMemory() - Runtime.freeMemory().
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public interface HeapProfiler {
  ...
  /**
   * Enables profiling for the specified application. Some implementations may require explicit
   * enablement for performance reasons.
   *
  ...
  /**
   * Checks if the JVMTI heap profiler is available. This method checks if the native agent library
