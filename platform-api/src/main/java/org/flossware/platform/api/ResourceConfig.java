...
/**
 * Configuration for application resource limits and enforcement policies. Uses convenient units
 * (MB, seconds) compared to ResourceQuota (bytes, nanos). All limits are optional - unconfigured
 * resources have no limits.
 *
 * <p>Resource enforcement allows automatic action when quotas are exceeded. Each resource type
 * (heap, CPU, threads) can have its own enforcement policy. Enforcement actions are triggered after
 * a configurable grace period to avoid transient spikes from causing unnecessary actions.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public final class ResourceConfig {
  ...
  /**
   * Returns the number of consecutive violations required before enforcement. This grace period
   * prevents transient resource spikes from triggering enforcement actions. For example, if set to
   * 3, the application must exceed its quota for 3 consecutive monitoring cycles (default: 15
   * seconds at 5-second intervals) before enforcement occurs.
   *
  ...
  /**
   * Builder for constructing ResourceConfig instances. All resource limits are optional.
   * Enforcement actions default to NOTIFY.
   */
  public static class Builder {
    ...
    public Builder maxCpuTimeSeconds(long seconds) {
      if (seconds <= 0) {
        throw new IllegalArgumentException(
            "Max CPU time seconds must be positive, got: " + seconds);
      }
    ...
    /**
     * Sets the enforcement action for memory quota violations. If not set, defaults to {@link
    ...
    /**
     * Sets the enforcement action for CPU quota violations. If not set, defaults to {@link
     * EnforcementAction#NOTIFY}.
     *
     * @param action the enforcement action (must not be null)
     * @return this builder
    ...
    /**
     * Sets the enforcement action for thread count quota violations. If not set, defaults to {@link
     * EnforcementAction#NOTIFY}.
     *
     * @param action the enforcement action (must not be null)
     * @return this builder
    ...
    /**
     * Sets the number of consecutive quota violations required before enforcement actions are
     * triggered. This provides a grace period for transient resource spikes.
     *
     * <p>Default is 3 violations, which equals 15 seconds at the default 5-second monitoring
     * interval.
     *
